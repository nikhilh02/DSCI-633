        ...
        if target:
            ...
            if average == "macro":
                ...
                for x in self.classes_:
                    prec = self.precision(target=x, average=average)
                    rec = self.recall(target=x, average=average)
                    if prec + rec == 0:
                        f1_score += 0
                    else:
                        f1_score += (2.0 * prec * rec / (prec + rec))
                f1_score = (f1_score) / len(self.classes_)
                ...
                for x in self.classes_:
                    ...
                    if prec + rec == 0:
                        ...
                        f1_score += ((2.0 * prec * rec / (prec + rec)) * Counter(self.actuals)[x])

                f1_score = (f1_score) / len(self.actuals)
            ...
            if target in self.classes_:
                ...
                for i in order:
                    if self.actuals[i] == target:
                        ...
                        fp = fp + 1  # "write your own code"
                        tn = tn - 1  # "write your own code"
                        pre_fpr = fpr
                        fpr = float(fp) / float(tn + fp)  # "write your own code"
                        auc_target = auc_target + tpr * (fpr - pre_fpr)  # "write your own code"
            else:
