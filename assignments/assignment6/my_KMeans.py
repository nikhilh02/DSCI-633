...
class my_KMeans:
    ...
    def initiate(self, X):
        ...
        if self.init == "random":
            randomList = np.random.permutation(rowCount)
            cluster_centers = X[randomList[:self.n_clusters]]
    ...
    def fit_once(self, X):
        ...
        for i in range(self.max_iter + 1):
            ...
            for x in X:
                ...
                closestValIndex = np.argmin(dists)
                # calculate inertia
                inertia += (dists[closestValIndex] ** 2)
                # find the cluster that x belongs to
                cluster_id = closestValIndex
            ...
            newCentroids = np.zeros((self.n_clusters, totalColumns))
            for clusterIndex in range(self.n_clusters):
                newCentroids[clusterIndex, :] = sum(clusters[clusterIndex]) / len(clusters[clusterIndex])

            cluster_centers = newCentroids
